...
A **vector** is a simple ordered collection, all items being of the same type (e.g., `num`). The **`c()`** function creates a vector.
...
A new syntax convention in the code below is the **pipe**, written as `%>%`. For `functionA() %>% functionB()`, you could read it as, "send the output of functionA to functionB."
...
-   The approach of mixing computer code with natural-language narrative is called "literate programming" (Knuth 1992)
...
-   Incorporates conversion tool [pandoc](https://pandoc.org/) to generate a wealth of output formats (HTML notebook, Word doc, Powerpoint, HTML presentation, PDF, latex...)
